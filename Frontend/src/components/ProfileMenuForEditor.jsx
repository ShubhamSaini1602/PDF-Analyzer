...
function ProfileMenuForEditor(){
    ...
    return (
        ...
        <div className="profile-menu-container2">
            {/* ProfileBtn */}
            <button className="profile-btn2" onClick={() => setIsOpen(!isOpen)}>
                {profilePic ? (
                    <img src={profilePic} className="profile-pic-2" alt="Profile"></img>
                ) : (
                    <div className="short-name-div2">
                        <p className="short-name2">{initials}</p>
                    </div>
                )}
                <p className="full-name2">{fullName}</p>
            </button>
            {/* ProfileDropDown */}
            <div className={`profile-dropdown profile-dropdown2 ${isOpen ? "open" : ""}`}>
...
export default ProfileMenuForEditor;